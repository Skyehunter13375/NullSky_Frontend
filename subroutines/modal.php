<?php
    ...
    switch ($desire) {
        case 'System':
            $SYST = SELECT("SELECT * FROM systems WHERE symbol = '{$symbol}'")[0];
            $WAYP = SELECT("SELECT system,symbol,type,x,y,orbits,construction FROM waypoints WHERE system = '{$symbol}' ORDER BY type");
            print("
                    <div class='panel'>
                        <table class='table'>
                            <thead>
            ...
            foreach ($WAYP as $key => $val) {
                if ($val['symbol'] == $symbol) { // BUGS This doesn't work anymore since I'm passing system instead of specific waypoint, may need another data point passed through
        ...
        case 'Waypoint':
            $WAYPNT = SELECT("SELECT * FROM waypoints WHERE symbol = '{$symbol}'")[0];
            $TRAITS = SELECT("SELECT
                                wayp.trait,
                                desc.name,
                                desc.description
                            FROM waypoint_traits AS wayp
                            JOIN waypoint_trait_descriptions AS desc ON wayp.trait = desc.symbol
                            WHERE wayp.waypoint = '{$symbol}'
            ");
            print("
                    <div class='panel'>
                        <h3 class='text-primary text-lg section-title'>{$WAYPNT['symbol']}</h3>
                        <table class='table'>
                            <thead>
                                <tr>
                                    <th>Type</th>
                            ...
                            <tbody>
                                <tr>
                                    <td>{$WAYPNT['type']}</td>
                                    <td>{$WAYPNT['orbits']}</td>
                                    <td>{$WAYPNT['x']},{$WAYPNT['y']}</td>
                                    <td>{$WAYPNT['construction']}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div><br>
                    ...
                    <div class='panel'>
                        <h3 class='text-primary text-lg section-title'>Traits</h3>
                        <table class='table'>
                            <thead>
                                <tr>
                                    <th>Name</th>
            ...
            print("
                    ...
                    <div class='panel'>
                        <h3 class='text-primary text-lg section-title'>Modifiers</h3>
                    </div>
            ");
            break;
